...
import numpy as np
import yaml
from dataset_reader_consep import DatasetReader
...
pred_train_out_dir = 'cla_cs_pred_circle_train_out_dir'
pred_valid_out_dir = 'cla_cs_pred_circle_valid_out_dir'
pred_test_out_dir = 'cla_cs_pred_circle_test_out_dir'
...
def main(NetClass):
    ...
    train_dataset = DatasetReader(os.path.join(project_root, 'consep_dataset'), 'train', pt_radius_min=1, pt_radius_max=1, rescale=0.5, use_repel_code=False)
    # valid_dataset = DatasetReader(os.path.join(project_root, 'consep_dataset'), 'test', pt_radius_min=1, pt_radius_max=1, rescale=0.5, use_repel_code=False)
    test_dataset = DatasetReader(os.path.join(project_root, 'consep_dataset'), 'test', pt_radius_min=1, pt_radius_max=1, rescale=0.5, use_repel_code=False)
    ...
    for heatmap_thresh in pred_heatmap_thresh_list:
        for did, cur_dataset in enumerate([train_dataset, test_dataset]):
            out_dir = [pred_train_out_dir, pred_test_out_dir][did]
            ...
            for pid in range(len(cur_dataset)):
                ...
                # 特别处理sp1，将正常类合并到肿瘤类，即将4合并到0
                if use_sp1_process:
                    for i in range(len(pred_cla_pts_cla)):
                        if pred_cla_pts_cla[i] == 4:
                            pred_cla_pts_cla[i] = 0

                    for i in range(len(label_cla_pts_cla)):
                        if label_cla_pts_cla[i] == 4:
                            label_cla_pts_cla[i] = 0
