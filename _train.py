...
from a_config import project_root, device, net_in_hw, net_out_hw, batch_size, epoch, batch_count,\
    is_train_from_recent_checkpoint, dataset_path, net_save_dir, net_train_logs_dir, is_ce_loss, train_lr,\
    match_distance_thresh_list, process_control
...
def calc_det_pix_f1(pred: np.ndarray, label: np.ndarray):
    '''
    只为了提高第一阶段的召回率
    :param pred:
    :param label:
    :return:
    '''
    assert pred.ndim == 4 and label.ndim == 4, 'pred.ndim {} ; label.ndim {}'.format(pred.ndim, label.ndim)
    assert pred.shape[3] == 1
    assert label.shape[3] == 1
    pred = (pred > 0.5).astype(dtype=np.float32)
    label = (label > 0.5).astype(dtype=np.float32)

    tp = (pred * label).sum(dtype=np.float32)
    tn = ((1-pred) * (1-label)).sum(dtype=np.float32)
    fp = (pred * (1-label)).sum(dtype=np.float32)
    fn = ((1-pred) * label).sum(dtype=np.float32)
...
def main(NetClass):
    ...
    det_train_dataset = DatasetReader(dataset_path, 'det', is_train=True, pt_radius_min=3, pt_radius_max=3)
    det_eval_dataset = DatasetReader(dataset_path, 'det', is_train=False, pt_radius_min=3, pt_radius_max=3)

    cla_train_dataset = DatasetReader(dataset_path, 'cla', is_train=True, pt_radius_min=3, pt_radius_max=3)
    cla_eval_dataset = DatasetReader(dataset_path, 'cla', is_train=False, pt_radius_min=3, pt_radius_max=3)
    ...
    start_epoch = 0
    process_max_valid_value = [-math.inf for _ in range(len(process_control)+1)]
    process_minloss_value = [math.inf for _ in range(len(process_control)+1)]
    ...
    if is_train_from_recent_checkpoint and os.path.isfile(ck_restart_name):
        ...
        process_max_valid_value = d['process_max_valid_value']
        process_minloss_value = d['process_minloss_value']
    ...
    for e in range(start_epoch, epoch):
        ...
        if e in process_control:
            new_ck_name = get_pg_name(ck_best_name, e - 1)
            new_ck_optim_name = get_pg_name(ck_optim_name, e - 1)
            net.load_state_dict(torch.load(new_ck_name, 'cpu'))
            optim.load_state_dict(torch.load(new_ck_optim_name, 'cpu'))
        ...
        if pg_id == 1:
            train_dataset = det_train_dataset
            eval_dataset = det_eval_dataset
            net.enabled_cls_branch = False
            net.set_freeze_seg1(False)
            net.set_freeze_seg2(True)
        elif pg_id == 2:
            train_dataset = cla_train_dataset
            eval_dataset = cla_eval_dataset
        ...
        for b in range(batch_count):
            ori_batch_im, ori_batch_label = next(train_gen)

            batch_im = torch.tensor(ori_batch_im, dtype=torch.float32, device=device).permute(0, 3, 1, 2) / 255
            batch_label = torch.tensor(ori_batch_label, dtype=torch.float32, device=device).permute(0, 3, 1, 2)

            if pg_id == 1:
                batch_label_det_pm = batch_label
            else:
                batch_label_det_pm = batch_label.max(1, keepdim=True)

            if is_ce_loss:
                batch_label = tr_pm_to_onehot_vec(batch_label)

            batch_pred_det, batch_pred_cla = net(batch_im)

            # 训练时检查像素准确率，这里的准确率按照类别平均检查
            with torch.no_grad():
                # batch_pred_cm = torch.argmax(batch_pred_pm, 1, keepdim=False).cpu()
                if is_ce_loss:
                    det_acc = ((batch_pred_det.softmax(1)[:, 1] > 0.5).type(torch.float32) * batch_label[:, 1]).sum() / (batch_label[:, 1].sum() + 1e-8)
                    cla_acc = 0.
                    if pg_id > 1:
                        cla_acc = ((batch_pred_cla.softmax(1)[:, 1:] > 0.5).type(torch.float32) * batch_label[:, 1:]).sum() / (batch_label[:, 1:].sum() + 1e-8)
                else:
                    det_acc = ((batch_pred_det > 0.5).type(torch.float32) * batch_label).sum() / (batch_label.sum() + 1e-8)
                    cla_acc = 0.
                    if pg_id > 1:
                        cla_acc = ((batch_pred_cla > 0.5).type(torch.float32) * batch_label).sum() / (batch_label.sum() + 1e-8)

                det_acc = det_acc.item()
                cla_acc = cla_acc.item() if isinstance(cla_acc, torch.Tensor) else cla_acc
                ...
                if time.time() - _last_auto_show_update_time > 60:
                    ...
                    if is_ce_loss:
                        show_ims(vtb, batch_label.mul(255).type(torch.uint8), 'label')
                        show_ims(vtb, batch_pred_det.softmax(1).mul(255).type(torch.uint8), 'det')
                        show_ims(vtb, (batch_pred_det.softmax(1)[:, 1:2] - batch_label_det_pm).abs().mul(255).type(torch.uint8), 'det_error')
                        if pg_id > 1:
                            show_ims(vtb, batch_pred_cla.softmax(1).mul(255).type(torch.uint8), 'cla')
                    else:
                        show_ims(vtb, batch_label.mul(255).type(torch.uint8), 'label')
                        show_ims(vtb, batch_pred_det.clamp(0, 1).mul(255).type(torch.uint8), 'det')
                        show_ims(vtb, (batch_pred_det.clamp(0, 1) - batch_label_det_pm).abs().mul(255).type(torch.uint8), 'det_error')
                        if pg_id > 1:
                            show_ims(vtb, batch_pred_cla.clamp(0, 1).mul(255).type(torch.uint8), 'cla')

            # loss 计算
            if is_ce_loss:
                if pg_id == 1:
                    loss = loss_func.det_loss(batch_pred_det.softmax(dim=1), batch_label, 'auto')
                elif pg_id == 2:
                    loss = loss_func.cla_loss(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1), batch_label, 'auto')
                elif pg_id == 3:
                    loss = loss_func.joint_loss(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1), batch_label)
                else:
        ...
        if (e+1) % 1 == 0:
            with torch.no_grad():
                ...
                for batch_im, batch_label in gen:
                    ...
                    batch_im = torch.tensor(batch_im, dtype=torch.float32, device=device).permute(0, 3, 1, 2) / 255
                    batch_label = torch.tensor(batch_label, dtype=torch.float32, device=device).permute(0, 3, 1, 2)
                    ...
                    if is_ce_loss:
                        if pg_id == 1:
                            loss = loss_func.det_loss(batch_pred_det.softmax(dim=1), batch_label)
                        elif pg_id == 2:
                            loss = loss_func.cla_loss(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1), batch_label)
                        elif pg_id == 3:
                            loss = loss_func.joint_loss(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1), batch_label)
                        else:
                            raise AssertionError('')
                    ...
                    if is_ce_loss:
                        batch_pred_det = batch_pred_det.softmax(1)[:, 1:].permute(0, 2, 3, 1).cpu().numpy()
                        batch_label = batch_label[:, 1:].permute(0, 2, 3, 1).cpu().numpy()
                        if pg_id > 1:
                            batch_pred_cla = batch_pred_cla.softmax(1)[:, 1:].permute(0, 2, 3, 1).cpu().numpy()
                    else:
                        batch_pred_det = batch_pred_det.permute(0, 2, 3, 1).cpu().numpy()
                        batch_label = batch_label.permute(0, 2, 3, 1).cpu().numpy()
                        if pg_id > 1:
                            batch_pred_cla = batch_pred_cla.permute(0, 2, 3, 1).cpu().numpy()

                    if pg_id > 1:
                        batch_pred_cla_final = batch_pred_det * batch_pred_cla
                    else:
                        batch_pred_cla_final = [None] * len(batch_pred_det)

                    # 统计像素分类数据
                    det_pix_pred_bin = (batch_pred_det > 0.5).astype(dtype=np.float32)
                    if pg_id == 1:
                        det_pix_label_bin = (batch_label > 0.5).astype(dtype=np.float32)
                    else:
                        det_pix_label_bin = np.max(batch_label > 0.5, axis=3, keepdims=True).astype(dtype=np.float32)
                    ...
                    det_score_table['det_pix_label_found'] = float(det_score_table['det_pix_pred_found'])
                    det_score_table['det_pix_label_nofound'] = float(((1 - det_pix_pred_bin) * det_pix_label_bin).sum(dtype=np.float32))

                    # 对每一个样本继续处理
                    # pi = 0
                    for each_label, each_pred_det, each_pred_cla in zip(batch_label, batch_pred_det, batch_pred_cla_final):
                        # pi += 1
                        # if pi > 3:
                        #     break
                        if pg_id == 1:
                            det_info = eval_utils.calc_a_sample_info_points_each_class(each_pred_det, each_label,
                                                                                       cls_list=[0, ],
                                                                                       match_distance_thresh_list=match_distance_thresh_list,
                                                                                       use_post_pro=True)
                            cla_info = None
                        else:
                            each_label_det = np.max(each_label, -1, keepdims=True)
                            # cv2.imshow('p1', each_pred_cla[..., :3])
                            # cv2.imshow('p2', each_pred_cla[..., 3:])
                            # cv2.imshow('dp1', each_pred_det)
                            # cv2.imshow('dl1', each_label_det)
                            det_info = eval_utils.calc_a_sample_info_points_each_class(each_pred_det, each_label_det,
                                                                                       cls_list=[0,],
                                                                                       match_distance_thresh_list=match_distance_thresh_list,
                                                                                       use_post_pro=True)

                            cla_info = eval_utils.calc_a_sample_info_points_each_class(each_pred_cla, each_label,
                                                                                       cls_list=DatasetReader.cla_class_ids,
                                                                                       match_distance_thresh_list=match_distance_thresh_list,
                                                                                       use_post_pro=True)
                        for dt in match_distance_thresh_list:
                            for cls in [0]:
                ...
                # 保存最好的
                if current_valid_value > process_max_valid_value[pg_id-1]:
                    process_max_valid_value[pg_id-1] = current_valid_value
                    new_ck_best_name = get_pg_name(ck_best_name, e)
                    ...
                    new_score_det_best_name = get_pg_name(score_det_best_name, e)
                    new_score_cla_best_name = get_pg_name(score_cla_best_name, e)
                ...
                if current_eval_loss < process_minloss_value[pg_id-1]:
                    process_minloss_value[pg_id-1] = current_eval_loss
                ...
                new_ck_name = get_pg_name(ck_name, e)
                new_ck_optim_name = get_pg_name(ck_optim_name, e)
                torch.save(net.state_dict(), new_ck_name)
                torch.save(optim.state_dict(), new_ck_optim_name)
                d = {'start_epoch': e + 1, 'process_max_valid_value': process_max_valid_value,
                     'process_minloss_value': process_minloss_value}
                yaml.safe_dump(d, open(ck_restart_name, 'w'))
                new_score_det_name = get_pg_name(score_det_name, e)
                new_score_cla_name = get_pg_name(score_cla_name, e)
