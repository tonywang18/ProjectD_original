...
def main(NetClass):
    ...
    train_dataset = DatasetReader(dataset_path, 'train', pt_radius_min=3, pt_radius_max=3, use_repel_code=use_repel_code)
    eval_dataset = DatasetReader(dataset_path, 'valid', pt_radius_min=3, pt_radius_max=3, use_repel_code=use_repel_code)
    ...
    for e in range(start_epoch, epoch):
        ...
        for b in range(batch_count):
            ...
            batch_label_mask = make_unmatched_mask(batch_label_det, batch_label_cla)
            ...
            with torch.no_grad():
                ...
                if is_ce_loss:
                    det_acc = ((batch_pred_det.softmax(1)[:, 1:2] > 0.5).type(torch.float32) * batch_label_det).sum() / (batch_label_det.sum() + 1e-8)
                    cla_acc = 0.
                    if pg_id > 1:
                        cla_acc = ((batch_pred_cla.softmax(1)[:, 1:] > 0.5).type(torch.float32) * batch_label_cla).sum() / (batch_label_cla.sum() + 1e-8)
                else:
                    det_acc = ((batch_pred_det.clamp(0., 1.) > 0.5).type(torch.float32) * batch_label_det).sum() / (batch_label_det.sum() + 1e-8)
                    cla_acc = 0.
                    if pg_id > 1:
                        cla_acc = ((batch_pred_cla.clamp(0., 1.) > 0.5).type(torch.float32) * batch_label_cla).sum() / (batch_label_cla.sum() + 1e-8)
                ...
                if time.time() - _last_auto_show_update_time > 60:
                    ...
                    #     show_ims(vtb, batch_pred_det.softmax(1).mul(255).type(torch.uint8), 'pred_det')
                    #     show_ims(vtb, (batch_pred_det.softmax(1)[:, 1:2] - batch_label_det).abs().mul(255).type(torch.uint8), 'det_error')
                    #     if pg_id > 1:
                    ...
                    #     show_ims(vtb, (batch_pred_det.clamp(0, 1) - batch_label_det).abs().mul(255).type(torch.uint8), 'det_error')
                    #     if pg_id > 1:
            ...
            # if is_ce_loss:
            #     if pg_id == 1:
            #         loss = loss_func.det_loss(batch_pred_det.softmax(dim=1), batch_label_det_oh, 'auto')
            #     elif pg_id == 2:
            #         loss = loss_func.cla_loss_v2(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1), batch_label_cla_oh, batch_label_mask, 'auto')
            #     elif pg_id == 3:
            #         loss = loss_func.joint_loss_v2(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1), batch_label_det_oh, batch_label_cla_oh, batch_label_mask)
            #     else:
            #         raise AssertionError('Unknow process_step')
            # else:
            #     if pg_id == 1:
            #         loss = loss_func.a_loss(batch_pred_det, batch_label_det, 'auto')
            #     elif pg_id == 2:
            #         loss = loss_func.a_loss(batch_pred_cla, batch_label_cla, 'auto')
            #     elif pg_id == 3:
            #         loss1 = loss_func.a_loss(batch_pred_det, batch_label_det, 'auto')
            #         loss2 = loss_func.a_loss(batch_pred_cla, batch_label_cla, 'auto')
            #         loss = loss1 + loss2
            #     else:
            #         raise AssertionError('Unknow process_step')
            if pg_id == 1:
                ...
                loss = loss_func.a_cla_loss(batch_pred_det, batch_pred_cla, batch_label_cla, 'auto')
            elif pg_id == 3:
                loss1 = loss_func.det_loss(batch_pred_det.softmax(dim=1), batch_label_det_oh, 'auto')
                loss2 = loss_func.a_cla_loss(batch_pred_det, batch_pred_cla, batch_label_cla, 'auto')
                loss = loss1 + loss2
            else:
                raise AssertionError('Unknow process_step')
        ...
        if (e+1) % 1 == 0:
            with torch.no_grad():
                ...
                # 分割相关
                for ori_batch_im, ori_batch_label in gen:
                    ...
                    batch_label_mask = make_unmatched_mask(batch_label_det, batch_label_cla)
                    ori_batch_label_mask = batch_label_mask.permute(0, 2, 3, 1).cpu().numpy()
                    ...
                    #         loss = loss_func.det_loss(batch_pred_det.softmax(dim=1), batch_label_det_oh, 'auto')
                    #     elif pg_id == 2:
                    #         loss = loss_func.cla_loss_v2(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1),
                    #                                      batch_label_cla_oh, batch_label_mask, 'auto')
                    #     elif pg_id == 3:
                    #         loss = loss_func.joint_loss_v2(batch_pred_det.softmax(dim=1), batch_pred_cla.softmax(dim=1),
                    #                                        batch_label_det_oh, batch_label_cla_oh, batch_label_mask)
                    #     else:
                    #         raise AssertionError('Unknow process_step')
                    # else:
                    #     if pg_id == 1:
                    ...
                    # else:
                    #     batch_pred_det = batch_pred_det.clamp(0, 1).permute(0, 2, 3, 1).cpu().numpy()
                    #     if pg_id > 1:
                    #         batch_pred_cla = batch_pred_cla.clamp(0, 1).permute(0, 2, 3, 1).cpu().numpy()
                    batch_pred_det = batch_pred_det.softmax(1)[:, 1:].permute(0, 2, 3, 1).cpu().numpy()
                    if pg_id > 1:
                        batch_pred_cla = batch_pred_cla.clamp(0, 1).permute(0, 2, 3, 1).cpu().numpy()
                    ...
                    if pg_id == 1:
                        det_pix_label_bin = (ori_batch_label_det > 0.5).astype(dtype=np.float32)
                        # 在阶段1不使用mask
                        ori_batch_label_mask[:] = 1
                    else:
                        det_pix_label_bin = np.max(ori_batch_label_cla > 0.5, axis=3, keepdims=True).astype(dtype=np.float32)
                    ...
                    # 对每一个样本继续处理
                    # pi = 0
                    for each_label_det, each_label_cla, each_label_mask, each_pred_det, each_pred_cla_final in\
                            zip(ori_batch_label_det, ori_batch_label_cla, ori_batch_label_mask, batch_pred_det, batch_pred_cla_final):
                        # pi += 1
                        # if pi > 3:
                        #     break
                        if pg_id == 1:
                            ...
                            # each_label_det_from_cla = np.max(each_label_cla, -1, keepdims=True)
                            ...
                            det_info = eval_utils.calc_a_sample_info_points_each_class(each_pred_det, each_label_det,
                                                                                       ...
                                                                                       use_post_pro=True)
                            # 屏蔽掉不匹配区域的分类检测
                            each_pred_cla_final = each_pred_cla_final * each_label_mask
                            each_label_cla = each_label_cla * each_label_mask
