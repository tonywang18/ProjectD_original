...
def main(NetClass):
    ...
    train_dataset = DatasetReader(os.path.join(project_root, 'pannuke_packed_dataset'), 'fold_1', pt_radius_min=3, pt_radius_max=3, rescale=0.5, use_repel_code=use_repel_code)
    eval_dataset = DatasetReader(os.path.join(project_root, 'pannuke_packed_dataset'), 'fold_2', pt_radius_min=3, pt_radius_max=3, rescale=0.5, use_repel_code=False)
    ...
    for e in range(start_epoch, epoch):
        ...
        for b in range(batch_count):
            ...
            ori_batch_label_cla = ori_batch_label
        ...
        if (e+1) % 1 == 0:
            with torch.no_grad():
                ...
                for t_i in progressbar(range(len(eval_dataset)), prefix='evaluate'):
                    ...
                    label_cla = label
                    label_det = np.max(label_cla, -1, keepdims=True)

                    group_label_cla_pts = info['label']

                    label_det_pts = []
                    for v in group_label_cla_pts.values():
