...
def main(NetClass):
    ...
    train_dataset = DatasetReader(dataset_path, 'fold_1', pt_radius_min=9, pt_radius_max=9, rescale=0.5, use_repel_code=use_repel_code)
    eval_dataset = DatasetReader(dataset_path, 'fold_2', pt_radius_min=9, pt_radius_max=9, rescale=0.5, use_repel_code=False)
    ...
    for e in range(start_epoch, epoch):
        ...
        if (e+1) % 1 == 0:
            with torch.no_grad():
                ...
                for t_i in tqdm(range(len(eval_dataset)), desc='evaluate'):
                    patch_count += 1
