...
from a_config import project_root, device, net_in_hw, net_out_hw, batch_size, epoch, batch_count,\
    ...
    b1_is_ce_loss, b1_loss_type, b2_is_ce_loss, b2_loss_type, b3_is_ce_loss, b3_loss_type, train_lr,\
...
# 4步走策略
del process_control
# process_control = [50, 100, 200, 250]
...
def main(NetClass):
    ...
    train_dataset = DatasetReader(os.path.join(project_root, 'pannuke_packed_dataset'), 'fold_1', pt_radius_min=9, pt_radius_max=9, rescale=0.5, use_repel_code=use_repel_code)
    eval_dataset = DatasetReader(os.path.join(project_root, 'pannuke_packed_dataset'), 'fold_2', pt_radius_min=9, pt_radius_max=9, rescale=0.5, use_repel_code=False)
    ...
    # 训练第四阶段会特别处理
    pg_max_valid_value[-1] = [-math.inf, -math.inf]
    pg_minloss_value[-1] = [math.inf, math.inf]
    ...
    for e in range(start_epoch, epoch):
        ...
        train_gen = train_dataset.get_train_batch_point_gen(batch_count, batch_size, use_enhance=True, window_hw=net_in_hw)
        train_gen = preload_generator(train_gen)

        for b in range(batch_count):
            ori_batch_im, ori_batch_label, ori_batch_info = next(train_gen)

            ori_batch_label_cla, _ = np.split(ori_batch_label, [5,], -1)
            ori_batch_label_det = np.max(ori_batch_label_cla, -1, keepdims=True)
            ...
            batch_label_cla_oh = tr_pm_to_onehot_vec(batch_label_cla, soft_label=use_repel_code)

            batch_pred_det, batch_pred_det2, batch_pred_cla = net(batch_im)
            ...
            with torch.no_grad():
                ...
                if b1_is_ce_loss:
                    ...
                    batch_pred_det_tmp = batch_pred_det_tmp.clamp(0., 1.).type(torch.float32)

                batch_pred_det_tmp_bin = (batch_pred_det_tmp > 0.5).type(torch.float32)

                det_acc = (batch_pred_det_tmp_bin * batch_label_det).sum() / (batch_label_det.sum() + 1e-8).item()

                if pg_id > 1:
                    batch_pred_cla_tmp = batch_pred_cla
                    if b3_is_ce_loss:
                        batch_pred_cla_tmp = batch_pred_cla_tmp.softmax(1)[:, 1:]
                    else:
                        batch_pred_cla_tmp = batch_pred_cla_tmp.clamp(0., 1.)
                    batch_pred_cla_tmp_bin = (batch_pred_cla_tmp > 0.5).type(torch.float32)
                    cla_acc = ((batch_pred_cla_tmp_bin * batch_label_cla).sum(dim=(0, 2, 3)) / (batch_label_cla.sum(dim=(0, 2, 3)) + 1e-8)).mean().item()

                    if b2_is_ce_loss:
                        batch_pred_det2_tmp = batch_pred_det2.softmax(1)[:, 1:]
                    else:
                ...
                if time.time() - _last_auto_show_update_time > 60:
                    ...
                    if pg_id > 1:
                        ...
                        show_ims(vtb, batch_pred_det2_tmp.mul(255).type(torch.uint8), 'pred_det2')
            ...
            if pg_id == 1:
                ...
                assert not b3_is_ce_loss
                _tmp = batch_pred_det.softmax(dim=1)[:, 1:2]
                loss = loss_func.a_cla_loss_type3(_tmp, batch_pred_det2, batch_label_det, 'auto')
                loss += loss_func.a_cla_loss_type3(_tmp, batch_pred_cla, batch_label_cla, 'auto')
            elif pg_id == 3 or pg_id == 4:
                assert b1_is_ce_loss
                assert not b2_is_ce_loss
                assert not b3_is_ce_loss
                loss1 = loss_func.det_loss(batch_pred_det.softmax(dim=1), batch_label_det_oh, 'auto')
                _tmp = batch_pred_det.softmax(dim=1)[:, 1:2]
                loss2 = loss_func.a_cla_loss_type3(_tmp, batch_pred_det2, batch_label_det, 'auto')
                loss2 += loss_func.a_cla_loss_type3(_tmp, batch_pred_cla, batch_label_cla, 'auto')
                loss = loss1 + loss2
            else:
        ...
        if (e+1) % 1 == 0:
            with torch.no_grad():
                ...
                for t_i in progressbar(range(len(eval_dataset)), prefix='evaluate'):
                    patch_count += 1

                    # if patch_count > 20:
                    #     break

                    im, label, info = eval_dataset.get_item_point(t_i, use_enhance=False, window_hw=None)

                    ori_batch_label_cla, _ = np.split(label, [5, ], -1)
                    ori_batch_label_det = np.max(ori_batch_label_cla, -1, keepdims=True)

                    del label

                    group_label_cla_pts = info['label']
                    ...
                    wim = big_pic_result.BigPicPatch(1+1+cla_class_num, [im], (0, 0), net_in_hw, (0, 0), 0, 0, custom_patch_merge_pipe=eval_utils.patch_merge_func, patch_border_pad_value=255)

                    gen = wim.batch_get_im_patch_gen(batch_size*3)
                    # 如果需要预载入的元素太少时时，使用 preload_generator 会显著降低性能，需要注意，不能盲目使用
                    ...
                    det_pix_label_bin = (ori_batch_label_det[..., 0:1] > 0.5).astype(dtype=np.float32)
                    ...
                    det_info = eval_utils.calc_a_sample_info_points_each_class(_tmp_pred_det_pm, [label_det_pts, [0]*len(label_det_pts)], [0], match_distance_thresh_list,
                                                                            use_post_pro=False, use_single_pair=True)

                    tmp_label_cla_without_ignore_pos = []
                    tmp_label_cla_without_ignore_cla = []
                    for k in group_label_cla_pts:
                        tmp_label_cla_without_ignore_pos.extend(group_label_cla_pts[k])
                        tmp_label_cla_without_ignore_cla.extend([k] * len(group_label_cla_pts[k]))
                ...
                elif pg_id == 2 or pg_id == 3:
                    current_valid_value = det_score_table[match_distance_thresh_list[0]]['f1'] + \
                                          cla_score_table[match_distance_thresh_list[0]]['f1']
                elif pg_id == 4:
                    current_valid_value = [det_score_table[match_distance_thresh_list[0]]['f1'], cla_score_table[match_distance_thresh_list[0]]['f1']]
                ...
                if pg_id != 4:
                    if current_valid_value > pg_max_valid_value[pg_id - 1]:
                        ...
                        # 因为过程4为过程3的继续训练，所以过程4初始时可以使用过程3的最好的分数记录
                        pg_max_valid_value[4 - 1][0] = det_score_table[match_distance_thresh_list[0]]['f1']
                        pg_max_valid_value[4 - 1][1] = cla_score_table[match_distance_thresh_list[0]]['f1']

                elif pg_id == 4:
                    # 第四阶段保存最佳模型时，会先从当前最佳权重文件中读取最佳权重，然后替换相关模型段，再保存.
                    # 第四阶段保存的优化器，因为选择保存哪段很麻烦，所以跳过不保存。
                    ib1 = current_valid_value[0] > pg_max_valid_value[pg_id - 1][0]
                    ib2 = current_valid_value[1] > pg_max_valid_value[pg_id - 1][1]
                    if ib1:
                        pg_max_valid_value[pg_id - 1][0] = current_valid_value[0]
                    if ib2:
                        pg_max_valid_value[pg_id - 1][1] = current_valid_value[1]
                    if ib1 or ib2:
                        new_ck_best_name = get_pg_name(ck_best_name, e, process_control)
                        new_score_det_best_name = get_pg_name(score_det_best_name, e, process_control)
                        new_score_cla_best_name = get_pg_name(score_cla_best_name, e, process_control)

                        tmp_best_net = NetClass(3, b1_out_dim, b2_out_dim, b3_out_dim)
                        if not os.path.isfile(new_ck_best_name):
                            # 如果是刚到第四阶段，此时本阶段是没有最佳权重的，则使用上一阶段最佳权重
                            old_new_ck_best_name = get_pg_name(ck_best_name, e-1, process_control)
                            tmp_best_net.load_state_dict(torch.load(old_new_ck_best_name, 'cpu'))
                        else:
                            tmp_best_net.load_state_dict(torch.load(new_ck_best_name, 'cpu'))
                        tmp_best_net.to(device)
                        if ib1:
                            tmp_best_net.load_seg2_state_dict(net.seg2_state_dict())
                            yaml.safe_dump(det_score_table, open(new_score_det_best_name, 'w'))
                        if ib2:
                            tmp_best_net.load_seg3_state_dict(net.seg3_state_dict())
                            yaml.safe_dump(cla_score_table, open(new_score_cla_best_name, 'w'))
                        torch.save(tmp_best_net.state_dict(), new_ck_best_name)
