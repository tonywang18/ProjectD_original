...
class DatasetReader:
    ...
    def __init__(self, dataset_path=os.path.join(project_root, 'pannuke_packed_dataset'), data_type='fold_1', pt_radius_min=3, pt_radius_max=3, use_repel_code=False, rescale=1., use_balance=True, *, include_cls=None):
        ...
        if isinstance(include_cls, str):
            include_cls = (include_cls,)
        elif isinstance(include_cls, Iterable):
            include_cls = tuple(include_cls)
        elif include_cls is not None:
            raise AssertionError(f'Error! Invalid param include_cls {include_cls}')
    ...
    def _build(self):
        ...
        # 是否只要其中某些组织
        if self.include_cls is not None:
            new_encode_im_list = []
            new_im_type_list = []
            new_label_list = []
            for i, im_type in enumerate(self._im_type_list):
                if im_type in self.include_cls:
                    new_encode_im_list.append(self._encode_im_list[i])
                    new_im_type_list.append(self._im_type_list[i])
                    new_label_list.append(self._label_list[i])
            self._encode_im_list = new_encode_im_list
            self._im_type_list = new_im_type_list
    ...
    def get_item_point(self, item_id, use_enhance=False, window_hw=None, sample_center=None):
        ...
        im_type = self._im_type_list[item_id]
        label_cls_conts = self._label_list[item_id]

        im = cv2.imdecode(encode_im[1], -1)
...
if __name__ == '__main__':
    ...
    ds = DatasetReader(data_type='fold_1', pt_radius_min=9, pt_radius_max=9, use_repel_code=True, rescale=0.5, include_cls=None)
