...
from my_py_lib.list_tool import list_multi_get_with_ids, list_multi_get_with_bool, list_multi_set_with_ids, list_multi_set_with_bool
from my_py_lib import contour_tool
from my_py_lib.point_tool import get_shortest_link_pair
...
def draw_hm_circle(ori_im: np.ndarray, pred_pts: np.ndarray, label_pts: np.ndarray, close_dist=6):
    ...
    for p_id, p_pt in enumerate(pred_pts):
        if len(label_pts) != 0:
            dists = np.linalg.norm(p_pt[None,] - label_pts, axis=1)
...
def class_map_to_contours2(pm, pred_cls_list, cls_prob, need_merge_cls_list, use_post_pro):
    ...
    for cls in pred_cls_list:
        contours = contour_tool.find_contours(bm[:, :, cls], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE, keep_invalid_contour=True)
...
def calc_a_sample_info_points_each_class(pred_pm, label_pm, cls_list, match_distance_thresh_list=(5, 7, 9, 11), use_post_pro=False, use_single_pair=False):
    ...
    if isinstance(pred_pm, list):
        ...
        pred_contours, pred_cls = class_map_to_contours2(pred_pm, cls_list, [0.5]*len(cls_list), {}, use_post_pro)
        pred_bboxes = np.array([contour_tool.make_bbox_from_contour(c) for c in pred_contours])
        pred_centers = (pred_bboxes[:, :2] + pred_bboxes[:, 2:]) / 2 if len(pred_bboxes) > 0 else []

    if isinstance(label_pm, list):
        label_centers, label_cls = label_pm
    else:
    ...
    pred_cls = np.int32(pred_cls).reshape([-1])
    label_cls = np.int32(label_cls).reshape([-1])
    ...
    if len(label_centers) == 0 or len(pred_centers) == 0:
        for cls in cls_list:
            ...
            for distance in match_distance_thresh_list:
                ...
                score_table[cls][distance]['fakefound_pred'] = int(np.sum(pred_cls == cls))
                score_table[cls][distance]['found_label'] = 0
                score_table[cls][distance]['nofound_label'] = int(np.sum(label_cls == cls))
    ...
    for cls in cls_list: # 遍历获取每个类别的中心坐标，无论是预测的还是label
        score_table[cls] = {}
        pred_selected_bools = np.array(pred_cls, np.int) == cls
        label_selected_bools = np.array(label_cls, np.int) == cls
        ...
        for distance in match_distance_thresh_list:
            ...
            label_found_count = np.zeros(len(selected_label_centers), np.int)
            pred_found_count = np.zeros(len(selected_pred_centers), np.int)
            ...
            if not use_single_pair:
                ...
                pred_pt_ids, label_pt_ids, _ = get_shortest_link_pair(selected_pred_centers, selected_label_centers, distance)
                for i in pred_pt_ids:
                    pred_found_count[i] = 1
                for i in label_pt_ids:
                    label_found_count[i] = 1

            found_pred = int((pred_found_count > 0).sum())
            fakefound_pred = int((pred_found_count == 0).sum())

            found_label = int((label_found_count > 0).sum())
            nofound_label = int((label_found_count == 0).sum())
...
#     a1 = cv2.circle(a.copy(), (256, 256), 50, (1, ), thickness=-1)
#     a2 = cv2.circle(a.copy(), (280, 256), 50, (2, ), thickness=-1)
#     contours1, clss1 = class_map_to_contours(a1, [1, 2], False)
#     contours2, clss2 = class_map_to_contours(a2, [1, 2], False)
#     contours = contours1 + contours2
#     clss = clss1 + clss2
#     contours = simple_contours(contours, 0)
#     b = simple_draw_contours(a, contours, 255)
#     iou = calc_iou_with_two_contours(contours[0], contours[1])
#     print(iou)
#     cv2.imshow('abc', b)
#
#     c = fusion_im_contours(a, contours, clss, {1: 255, 2: 128})
#     cv2.imshow('c', c)
#
#     d = calc_a_sample_info(a1, a2, [1, 2])
#     print(d)
#
#     cv2.waitKey(0)
#     print(contours, clss)
