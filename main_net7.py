...
class MainNet(nn.Module):
    ...
    def __init__(self, in_dim=3, b1_out_dim=2, b2_out_dim=5):
        ...
        self.b1_conv1 = ConvBnAct(64, 64, 1, 1, 0, act)
        ...
        self.b2_conv1 = ConvBnAct(128, 128, 3, 1, 1, act)
        self.b2_conv2 = nn.Sequential(
            PsConvBnAct(4, 128, 128, 5, 1, 2),
            nn.Conv2d(128, b1_out_dim, 1, 1, 0, bias=True)
        )

        self._bm1.extend([self.conv1, self.rvb1, self.rb2, self.rvb2, self.rb3, self.rvb3, self.b1_conv1, self.b1_conv2, self.b2_conv1, self.b2_conv2])

        # self.rb4 = ResBlockA(128, 128, 2, act)
        self.rvb4 = RevGroupBlock(128, 128, 1, act, RevBlockC, 9)

        self.b4_conv1 = ConvBnAct(128, 128, 1, 1, 0, act)
        self.b4_conv2 = nn.Sequential(
            PsConvBnAct(4, 128, 128, 5, 1, 2),
            nn.Conv2d(128, b2_out_dim, 1, 1, 0, bias=True)
        )

        self._bm2.extend([self.rvb4, self.b4_conv1, self.b4_conv2])
